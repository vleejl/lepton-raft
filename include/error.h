...
template <err_types err_type>
bool operator==(const lepton_error<err_type>& error, const err_type& code) {
  return error.err_code == code;
}

template <err_types err_type>
bool operator==(const err_type& code, const lepton_error<err_type>& error) {
...
template <typename error_code_type, typename error_msg_type>
auto new_error(
