...
class unstable {
 ...
 public:
  ...
  void stable_to(std::uint64_t i, std::uint64_t t) {
    auto result = maybe_term(i);
    if (!result) {
      return;
    }

    if ((result.value() == t) && (i >= offset_)) {
      auto index = static_cast<std::ptrdiff_t>(i + 1 - offset_);
      assert(entries_.begin() + index < entries_.end());
      ...
      // no need shrinkEntriesArray beacasue of new std::vector
    }
  }
  ...
  void truncate_and_append(std::vector<pb::entry_ptr>&& entries) {
    assert(!entries.empty());
    auto after = entries[0]->index();
    if (after == (offset_ + entries_.size())) {  // max after value
      entries_.insert(entries.end(), std::make_move_iterator(entries.begin()),
