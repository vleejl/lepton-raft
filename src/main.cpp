...
awaitable<void> product_tick(steady_timer::duration interval, message_channel& chann,
                             lepton::signal_channel& close_chan) {
...
awaitable<void> close_message(message_channel& chann, lepton::signal_channel& close_chan) {
  // 等待关闭信号
...
awaitable<void> check_counter(lepton::signal_channel& close_chan) {
  while (true) {
    // 每秒检查一次计数器
    steady_timer timer(co_await this_coro::executor);
...
awaitable<void> run(io_context& ctx) {
  ...
  lepton::signal_channel stop_chan{ctx.get_executor()};
