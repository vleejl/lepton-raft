...
leaf::result<pb::repeated_entry> memory_storage::entries(
  ...
  if (hi > last_log_index + 1) {
    spdlog::critical("entries' hi(%d) is out of bound lastindex(%d)",
                     last_log_index);
  }
...
leaf::result<raftpb::snapshot> memory_storage::create_snapshot(
  ...
  if (i > _last_index()) {
    spdlog::critical("snapshot %d is out of bound lastindex(%d)", i,
                     _last_index());
  }
  const auto offset = ents_[0].index();
...
leaf::result<void> memory_storage::compact(std::uint64_t compact_index) {
  ...
  if (compact_index > _last_index()) {
    spdlog::critical("compact %d is out of bound lastindex(%d)", compact_index,
                     _last_index());
  }
...
leaf::result<void> memory_storage::append(pb::repeated_entry&& entries) {
  ...
  if (offset < ents_.size()) {
    ...
    spdlog::critical("offset({}) is out of range [len({})]", offset,
                     ents_.size());
  }
  return {};
}
