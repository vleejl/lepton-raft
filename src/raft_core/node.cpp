...
asio::awaitable<void> node::stop() {
  ...
  LOG_INFO("{} ready to send stop signal", id);
  if (stop_source_.stop_requested()) {
    // Node has already been stopped - no need to do anything
    LOG_INFO("{} node has already been stopped, just return", id);
    co_return;
  }
  ...
  LOG_INFO("{} Block until the stop has been acknowledged by run()", id);
  ...
  LOG_INFO("{} receive done siganl and stop has been acknowledged by run()", id);
  co_return;
}
...
asio::awaitable<void> node::run() {
  ...
  while (is_running()) {
    ...
    LOG_TRACE("run main loop, continue main loop logic ......");
    if (lead != r.lead()) {
      if (r.has_leader()) {
        if (lead == NONE) {
          LOG_TRACE("raft.node: {} elected leader {} at term {}", r.id(), r.lead(), r.term());
        } else {
          LOG_TRACE("raft.node: {} changed leader from {} to {} at term {}", r.id(), lead, r.lead(), r.term());
        }
        is_active_prop_chan = true;
      } else {
        LOG_TRACE("raft.node: {} lost leader {} at term {}", r.id(), lead, r.term());
    ...
    if (is_active_prop_chan) {
      LOG_TRACE("ready to send active prop channel");
      // 不能阻塞主循环
      active_prop_chan.try_send();
      LOG_TRACE("send active prop channel successful");
    }
    co_await token_chan.async_receive();
  }
  co_await waiter->wait_all();
  LOG_DEBUG("{} receive stop signal and exit run loop", r.id());
...
void node::tick() {
  ...
  if (!tick_chan_.raw_channel().try_send(asio::error_code{})) {
    LOG_WARN("{} A tick missed to fire. Node blocks too long!", raw_node_.raft_.id());
  }
}
...
asio::awaitable<expected<ready_handle>> node::wait_ready(asio::any_io_executor executor) {
  ...
  LOG_TRACE("prepare to request ready");
  auto result = co_await ready_request_chan_.async_send(std::weak_ptr(receive_ready_chan));
  if (!result) {
    LOG_ERROR("send callback channel failed, error:{}", result.error().message());
    co_return tl::unexpected{result.error()};
  }
  LOG_TRACE("request ready callback channel successful and prepare wait callback channel active");
  auto ready = co_await receive_ready_chan->async_receive();
  if (!ready) {
    LOG_ERROR("receive error when try to receive ready, error:{}", ready.error().message());
  } else {
    LOG_TRACE("callback channel actived and receive ready content:\n{}", describe_ready(*ready.value(), nullptr));
  }
  co_return ready;
}
...
asio::awaitable<void> node::advance() {
  ...
  if (!ec.has_value()) {
    LOG_ERROR("Failed to recv non-proposal message: {}", ec.error().message());
  }
  co_return;
}
...
asio::awaitable<void> node::listen_ready(coro::signal_channel_endpoint& token_chan,
  ...
  while (is_running()) {
    LOG_TRACE("waiting active_ready_chan signal......");
    if (auto result = co_await active_ready_chan.async_receive(); !result) {
      LOG_ERROR("Failed to receive active_ready_chan, error: {}", result.error().message());
      break;
    }
    LOG_TRACE("receive active_ready_chan signal");
    // 走到这里表面一定有 ready
    auto result = co_await ready_request_chan_.async_receive();
    if (!result) {
      LOG_ERROR("Failed to receive ready_request_chan, error: {}", result.error().message());
      break;
    }
    if (auto chan = result->lock()) {
      auto rd = std::make_shared<ready>(raw_node_.ready_without_accept());
      LOG_TRACE("ready to send ready by ready_channel, {}", describe_ready(*rd.get(), nullptr));
      ...
      LOG_TRACE("send ready by ready_channel successful, {}", describe_ready(*rd.get(), nullptr));
      if (!raw_node_.async_storage_writes()) {
        ...
        if (auto ec = co_await advance_chan_.async_receive(); !ec) {
          LOG_ERROR("Failed to receive advance, error: {}", ec.error().message());
          break;
        }
        LOG_TRACE("async_receive advance by advance_chan_ successfully");
        raw_node_.advance();
      ...
      LOG_TRACE("finish send ready by ready_channel successfully");
    } else {
      LOG_TRACE("callback channel has been released, wait next loop");
      co_await token_chan.async_send();
    }
  }
  LOG_INFO("[listen func]ready_request_chan_ closed, exit listen_ready loop");
  co_return;
}
...
asio::awaitable<void> node::listen_propose(coro::signal_channel_endpoint& token_chan,
  ...
  while (is_running()) {
    if (!is_active_prop_chan) {
      LOG_TRACE("waiting active_prop_chan signal......");
      co_await active_prop_chan.async_receive();
      LOG_TRACE("receive active_prop_chan signal");
    }
    LOG_TRACE("ready to listen prop_chan");
    auto msg_result = co_await prop_chan_.async_receive();
    if (!msg_result) {
      LOG_ERROR(msg_result.error().message());
      break;
    }

    auto& msg = msg_result->msg;
    LOG_TRACE("receive msg by prop_chan and ready to process, {}", msg.DebugString());
    ...
    if (msg_result->err_chan.has_value()) {
      ...
      if (auto chan = msg_result->err_chan->lock()) {
        ...
        LOG_WARN("try lock err_chan callback channel failed");
      }
    }
    LOG_TRACE("prop_chan_ send token_chan and wait next loop", msg.DebugString());
    co_await token_chan.async_send();
  }
  LOG_INFO("[listen func]prop_chan_ closed, exit listen_propose loop");
  co_return;
}

asio::awaitable<void> node::listen_receive(coro::signal_channel_endpoint& token_chan) {
  while (is_running()) {
    LOG_TRACE("begin recv_chan_ async_receive....");
    ...
    auto& msg = *result;
    LOG_TRACE("recv_chan_ async_receive receive msg. {}", msg.DebugString());
    if (pb::is_response_msg(msg.type()) && !pb::is_local_msg_target(msg.from()) &&
        !raw_node_.raft_.has_trk_progress(msg.from())) {
      LOG_TRACE("message type: {}, msg from: {}. Filter out response message from unknown From", enum_name(msg.type()),
                msg.from());
      // Filter out response message from unknown From.
    } else {
      auto _ = raw_node_.raft_.step(std::move(msg));
    }
    LOG_TRACE("recv_chan_ send token_chan and wait next loop");
    co_await token_chan.async_send();
    LOG_TRACE("send token signal successful");
  }
  LOG_INFO("[listen func]recv_chan_ closed, exit listen_receive loop");
  co_return;
}
...
asio::awaitable<void> node::listen_conf_change(coro::signal_channel_endpoint& token_chan, bool& is_active_prop_chan) {
  ...
  while (is_running()) {
    ...
    if (!ec.has_value()) {
      LOG_ERROR("Failed to send conf state, error: {}", ec.error().message());
    }
    LOG_TRACE("conf_chan_ send token_chan and wait next loop");
    co_await token_chan.async_send();
  }
  LOG_INFO("[listen func]conf_chan_ closed, exit listen_conf_change loop");
  co_return;
}
...
asio::awaitable<void> node::listen_tick(coro::signal_channel_endpoint& token_chan) {
  while (is_running()) {
    ...
    raw_node_.tick();
    LOG_TRACE("tick_chan_ send token_chan and wait next loop");
    co_await token_chan.async_send();
  }
  LOG_INFO("[listen func]tick_chan_ closed, exit listen_tick loop");
  co_return;
}
...
asio::awaitable<void> node::listen_status(coro::signal_channel_endpoint& token_chan) {
  while (is_running()) {
    ...
    if (auto chan = result->lock()) {
      ...
      LOG_TRACE("status_chan send token_chan and wait next loop");
    } else {
      LOG_WARN("try lock status callback channel failed");
    }
    co_await token_chan.async_send();
  }
  LOG_INFO("[listen func]status_chan_ closed, exit listen_status loop");
  co_return;
}

asio::awaitable<void> node::listen_stop() {
  co_await stop_chan_.async_receive();
  LOG_INFO("receive stop signal and stop all channels");
  ...
  tick_chan_.close();
  status_chan_.close();
  LOG_INFO("ready to send cancel and done signal");
  assert(done_chan_.is_open());
  co_await done_chan_.async_send(asio::error_code{});
  LOG_INFO("send done signal successful");
  LOG_INFO("[listen func]close done chan successful and exit listen_stop");
  co_return;
}
...
asio::awaitable<expected<void>> node::handle_non_prop_msg(raftpb::Message&& msg) {
  ...
  LOG_TRACE("send non-proposal message {} successful", debugMsg);
  co_return ok();
}
...
asio::awaitable<expected<void>> node::step_impl(raftpb::Message&& msg) {
  ...
  if (!ec.has_value()) {
    LOG_ERROR("Failed to handle non-proposal message: {}", ec.error().message());
  }
  co_return ec;
}
...
node_handle setup_node(asio::any_io_executor executor, lepton::core::config&& config, std::vector<peer>&& peers) {
  ...
  auto _ = leaf::try_handle_some(
      ...
      [&](const lepton_error& e) -> leaf::result<void> {
        LOG_WARN("error occurred during starting a new node: {}", e.message);
        return {};
      });
