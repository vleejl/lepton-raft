...
void release_pending_read_index_message(raft& r) {
  ...
  if (!r.committed_entry_in_current_term()) {
    LOGGER_ERROR(r.logger_, "pending MsgReadIndex should be released only after first commit in current term");
    return;
  }
...
leaf::result<void> step_leader(raft& r, raftpb::Message&& m) {
  ...
  switch (msg_type) {
    ...
    case raftpb::MSG_CHECK_QUORUM: {
      ...
      if (!r.trk_.quorum_active()) {
        ...
        LOGGER_WARN(r.logger_, "{} stepped down to follower since quorum is not active", r.id_);
        r.become_follower(r.term_, NONE);
      }
  ...
  if (pr_iter == r.trk_.progress_map_mutable_view().mutable_view().end()) {
    ...
    // was removed from the configuration while serving as leader),
    // drop any new proposals.
    LOGGER_DEBUG(r.logger_, "{} no progress available for {}", r.id_, msg_from);
    LOGGER_INFO(r.logger_, "{} [term {}] ignoring message from unknown node {}", r.id_, r.term_, msg_from);
    return {};
  }
  ...
  switch (msg_type) {
    ...
    case raftpb::MSG_APP_RESP: {
      ...
      if (m.reject()) {
        ...
        LOGGER_DEBUG(r.logger_, "{} received MsgAppResp(rejected, hint: (index {}, term {})) from {} for index {}",
                     r.id_, m.reject_hint(), m.log_term(), msg_from, m.index());
        ...
        if (pr.maybe_decr_to(m.index(), next_probe)) {
          LOGGER_DEBUG(r.logger_, "{} decreased progress of {} to [{}]", r.id_, msg_from, pr.string());
        ...
        if (pr.maybe_update(m.index()) || (pr.match() == m.index() && pr.state() == tracker::state_type::STATE_PROBE)) {
          if (pr.state() == tracker::state_type::STATE_PROBE) {
            ...
            LOGGER_DEBUG(r.logger_,
                         "{} recovered from needing snapshot, resumed sending replication messages to {} [{}]", r.id_,
                         msg_from, pr.string());
        ...
        if ((msg_from == r.leader_transferee_) && (pr.match() >= r.raft_log_handle_.last_index())) {
          LOGGER_INFO(r.logger_, "{} sent MsgTimeoutNow to {} after received MsgAppResp", r.id_, msg_from);
    ...
    case raftpb::MSG_SNAP_STATUS: {  // 用于管理快照发送后的节点状态转换和流量控制
      if (pr.state() != tracker::state_type::STATE_SNAPSHOT) {
        LOGGER_DEBUG(r.logger_, "{} [term {}] ignoring MsgSnapStatus from {} in state {}", r.id_, r.term_, msg_from,
                     enum_name(pr.state()));
        return {};
      }
      if (!m.reject()) {
        pr.become_probe();
        LOGGER_DEBUG(r.logger_, "{} snapshot succeeded, resumed sending replication messages to {} [{}]", r.id_,
                     msg_from, pr.string());
      } else {
        ...
        // 进入探测状态，重新尝试日志同步。
        pr.become_probe();
        LOGGER_DEBUG(r.logger_, "{} snapshot failed, resumed sending replication messages to {} [{}]", r.id_, msg_from,
                     pr.string());
      }
    ...
    case raftpb::MSG_UNREACHABLE: {  // 处理不可达节点的消息
      ...
      LOGGER_DEBUG(r.logger_, "{} failed to send message to {} because it is unreachable [{}]", r.id_, msg_from,
                   pr.string());
      break;
    }
    case raftpb::MSG_TRANSFER_LEADER: {
      if (r.is_learner_) {
        LOGGER_DEBUG(r.logger_, "{} is learner. Ignored transferring leadership", r.id_);
        return {};
      }
      ...
      if (last_leader_transferee != NONE) {
        if (leader_transferee == last_leader_transferee) {
          LOGGER_DEBUG(r.logger_,
                       "{} [term {}] transfer leadership to {} is in progress, ignores request to same node {}", r.id_,
                       r.term_, last_leader_transferee, leader_transferee);
          return {};
        }
        r.abort_leader_transfer();
        LOGGER_INFO(r.logger_, "{} [term {}] abort transfer leadership to {} and transfer to {}", r.id_, r.term_,
                    last_leader_transferee, leader_transferee);
      }
      if (leader_transferee == r.id_) {
        LOGGER_DEBUG(r.logger_, "{} [term {}] transfer leadership to self, ignores request", r.id_, r.term_);
        return {};
      }
      // Transfer leadership to third party.
      LOGGER_INFO(r.logger_, "{} [term {}] starts to transfer leadership to {}", r.id_, r.term_, leader_transferee);
      ...
      if (pr.match() == r.raft_log_handle_.last_index()) {
        ...
        LOGGER_INFO(r.logger_, "{} sends MsgTimeoutNow to {} immediately as {} already has up-to-date log", r.id_,
    ...
    default:
      LOGGER_TRACE(r.logger_, "{} [term {}] ignoring message from {} in state {}", r.id_, r.term_, msg_from,
                   enum_name(pr.state()));
      break;
  }
  return {};
...
leaf::result<void> step_candidate(raft& r, raftpb::Message&& m) {
  ...
  auto post_vote_resp_func = [&]() {
    ...
    LOGGER_INFO(r.logger_, "{} has received {} {} votes and {} vote rejections", r.id_, gr, enum_name(m.type()), rj);
    switch (res) {
  ...
  switch (msg_type) {
    case raftpb::MSG_PROP: {
      LOGGER_INFO(r.logger_, "{} no leader at term {}; dropping proposal", r.id_, r.term_);
      return new_error(raft_error::PROPOSAL_DROPPED);
    }
    case raftpb::MSG_APP: {
...
leaf::result<void> step_follower(raft& r, raftpb::Message&& m) {
  switch (m.type()) {
    ...
    case raftpb::MSG_TRANSFER_LEADER: {
      if (r.lead_ == NONE) {
        LOGGER_INFO(r.logger_, "{} no leader at term {}; dropping leader transfer msg", r.id_, r.term_);
        return {};
      }
      m.set_to(r.lead_);
      r.send(std::move(m));
    ...
    case raftpb::MSG_READ_INDEX: {
      if (r.lead_ == NONE) {
        LOGGER_INFO(r.logger_, "{} no leader at term {}; dropping index reading msg", r.id_, r.term_);
        return {};
      }
      m.set_to(r.lead_);
      r.send(std::move(m));
    ...
    case raftpb::MSG_READ_INDEX_RESP: {
      if (m.entries_size() != 1) {
        LOGGER_INFO(r.logger_, "{} invalid format of MsgReadIndexResp from {}", r.id_, m.from());
        return {};
      }
    ...
    case raftpb::MSG_FORGET_LEADER: {  // 强制 Follower 忘记当前
      ...
      if (r.read_only_.read_only_opt() == read_only_option::READ_ONLY_LEASE_BASED) {
        ...
        LOGGER_ERROR(r.logger_, "ignoring MsgForgetLeader due to ReadOnlyLeaseBased", r.id_, r.term_);
        return {};
      }
      if (r.lead_ != NONE) {
        LOGGER_INFO(r.logger_, "{} forgetting leader {} at term {}", r.id_, r.lead_, r.term_);
...
bool raft::maybe_send_snapshot(std::uint64_t to, tracker::progress& pr) {
  ...
  LOGGER_DEBUG(logger_, "{} [firstindex: {}, commit: {}] sent snapshot[index: {}, term: {}] to {} [{}]", id_,
  ...
  if (trk_.config_view().auto_leave && new_applied >= pending_conf_index_ && state_type_ == state_type::LEADER) {
    ...
    auto _ = leaf::try_handle_some(
        [&]() -> leaf::result<void> {
          ...
          LOGGER_INFO(logger_, "initiating automatic transition out of joint configuration {}",
                      trk_.config_view().string());
          return {};
        },
        [&](const lepton::lepton_error& err) -> leaf::result<void> {
          LEPTON_UNUSED(err);
          LOGGER_DEBUG(logger_, "not initiating automatic transition out of joint configuration {}: {}",
...
bool raft::append_entry(pb::repeated_entry&& entries) {
  ...
  if (!increase_uncommitted_size(entries)) {
    LOGGER_WARN(logger_, "{} appending new entries to log would exceed uncommitted entry size limit; dropping proposal",
                id_);
    // Drop the proposal.
    return false;
  }
...
void raft::tick_election() {
  ...
  if (promotable() && past_election_timeout()) {
    ...
    auto _ = leaf::try_handle_some(
        ...
        [&](const lepton::lepton_error& err) -> leaf::result<void> {
          LEPTON_UNUSED(err);
          LOGGER_DEBUG(logger_, "error occurred during election: {}", err.message);
          return {};
        });
  }
}
...
void raft::tick_heartbeat() {
  ...
  if (election_elapsed_ >= election_timeout_) {
    ...
    if (check_quorum_) {
      ...
      auto _ = leaf::try_handle_some(
          ...
          [&](const lepton::lepton_error& err) -> leaf::result<void> {
            LEPTON_UNUSED(err);
            LOGGER_DEBUG(logger_, "{} failed to send check quorum message: {}", id_, err.message);
            return {};
          });
    }
    ...
    if ((state_type_ == state_type::LEADER) && (leader_transferee_ != NONE)) {
      LOGGER_DEBUG(logger_, "{} [term {}] leader transfer timeout, become leader again", id_, term_);
      abort_leader_transfer();
    }
  }
  ...
  if (heartbeat_elapsed_ >= heartbeat_timeout_) {
    ...
    auto _ = leaf::try_handle_some(
        ...
        [&](const lepton::lepton_error& err) -> leaf::result<void> {
          LEPTON_UNUSED(err);
          LOGGER_DEBUG(logger_, "{} error occurred during checking sending heartbeat: {}", id_, err.message);
          return {};
        });
  }
}
...
void raft::become_follower(std::uint64_t term, std::uint64_t lead) {
  ...
  LOGGER_INFO(logger_, "{} became follower at term {}", id_, term_);
...
void raft::become_candidate() {
  ...
  LOGGER_INFO(logger_, "{} became candidate at term {}", id_, term_);
...
void raft::become_pre_candidate() {
  ...
  LOGGER_INFO(logger_, "{} became pre-candidate at term {}", id_, term_);
}
...
void raft::become_leader() {
  ...
  LOGGER_INFO(logger_, "{} became leader at term {}", id_, term_, term_);
}
...
void raft::hup(campaign_type t) {
  if (state_type_ == state_type::LEADER) {
    LOGGER_DEBUG(logger_, "{} [term {}] ignoring MsgHup because already leader", id_, term_);
    return;
  }

  if (!promotable()) {
    LOGGER_WARN(logger_, "{} is unpromotable and can not campaign", id_);
    return;
  }
  if (has_unapplied_conf_change()) {
    LOGGER_WARN(logger_, "{} cannot campaign at term {} since there are still pending configuration changes to apply",
                id_, term_);
    return;
  }
  LOGGER_INFO(logger_, "{} is starting a new election at term {}", id_, term_);
  campaign(t);
}
...
void raft::campaign(campaign_type t) {
  ...
  for (const auto& id : ids) {
    ...
    LOGGER_INFO(logger_, "{} [logterm: {}, index: {}] sent {} request to {} at term {}", id_, last.term, last.index,
...
std::tuple<std::uint64_t, std::uint64_t, quorum::vote_result> raft::poll(std::uint64_t id, raftpb::MessageType vt,
  ...
  if (vote) {
    ...
    LOGGER_INFO(logger_, "{} received {} rejection from {} at term {}", id_, enum_name(vt), id, term_);
  }
...
leaf::result<void> raft::step(raftpb::Message&& m) {
  ...
  if (m.term() == 0) {
    ...
    if (m.type() == raftpb::MessageType::MSG_PRE_VOTE) {
      ...
      // term.
      LOGGER_TRACE(logger_, "{} [term: {}] received PreVoteResp from {} [term: {}, index: {}]", id_, term_, m.from(),
                   m.term(), m.index());
    } else {
    ...
    if ((check_quorum_ || pre_vote_) &&
      ...
      if (m.index() != 0) {
        ...
        // about this race.
        LOGGER_INFO(logger_, "{} [term: {}] ignored entry appends from a {} message with lower term [term: {}]", id_,
      ...
      // ignore other cases
      LOGGER_INFO(logger_, "{} [term: {}] ignored a {} message with lower term from {} [term: {}]", id_, term_,
                  enum_name(m.type()), m.from(), m.term());
    }
    return {};
  }
  ...
  switch (m.type()) {
    ...
    case raftpb::MessageType::MSG_PRE_VOTE: {
      ...
      if (can_vote && raft_log_handle_.is_up_to_date(cand_last_id)) {
        ...
        LOGGER_INFO(logger_, "{} [logterm: {}, index: {}, vote: {}] cast {} for {} [logterm: {}, index: {}] at term {}",
                    id_, last.term, last.index, vote_id_, enum_name(m.type()), m.from(), m.log_term(), m.index(),
                    term_);
...
void raft::handle_append_entries(raftpb::Message&& message) {
  ...
  // 本地追加日志失败：则发送拒绝消息
  LOGGER_DEBUG(logger_, "{} [logterm: {}, index: {}] rejected MsgApp [logterm: {}, index: {}] from {}", id_,
...
void raft::handle_snapshot(raftpb::Message&& message) {
  ...
  if (message.has_snapshot()) {
    ...
    LOGGER_WARN(logger_, "{} [commit: {}] unreachable case", id_, raft_log_handle_.committed());
  }
  ...
  if (restore(std::move(snapshot))) {
    ...
    LOGGER_INFO(logger_, "{} [commit: {}] ignored snapshot [index: {}, term: {}]", id_, raft_log_handle_.committed(),
                sindex, sterm);
    resp_msg.set_index(raft_log_handle_.committed());
  }
  send(std::move(resp_msg));
...
bool raft::restore(raftpb::Snapshot&& snapshot) {
  ...
  if (state_type_ != state_type::FOLLOWER) {
    ...
    LOGGER_WARN(logger_, "{} attempted to restore snapshot as leader; should never happen", id_);
  ...
  if (!snapshot.has_metadata() || !snapshot.metadata().has_conf_state()) {
    LOGGER_WARN(logger_, "{} attempted to restore snapshot with no ConfState; should never happen", id_);
    return false;
  }
  ...
  if (!found) {
    LOGGER_WARN(logger_, "{} attempted to restore snapshot but it is not in the ConfState {}; should never happen", id_,
                cs.DebugString());
    return false;
  }
  ...
  auto last = raft_log_handle_.last_entry_id();
  LOGGER_INFO(logger_, "{} [commit: {}, lastindex: {}, lastterm: {}] restored snapshot [index: {}, term: {}]", id_,
...
raftpb::ConfState raft::switch_to_config(tracker::config&& cfg, tracker::progress_map&& pgs_map) {
  ...
  LOGGER_INFO(logger_, "{} switched to configuration {}", id_, trk_.config_view().string());
