...
asio::awaitable<expected<void>> decoder::decode_record_impl(walpb::record& rec) {
  ...
  auto file_buf_reader = readers_.front();
  auto read_result = co_await read_uint64(file_buf_reader);
  ...
  auto file_size_result = leaf_to_expected([&]() -> leaf::result<std::size_t> {
    BOOST_LEAF_AUTO(file_size, file_buf_reader->size());
    return file_size;
  });
  if (!file_size_result) {
  ...
  if (rec_bytes > max_entry_limit) {
    LOG_ERROR(logger_,
              ...
              "padBytes({}) = entryLimit({})",
              file_buf_reader->name(), rec_bytes, file_size, last_valid_off_, pad_bytes, max_entry_limit);
    co_return tl::unexpected(io_error::UNEXPECTED_EOF);
  }

  fixed_byte_buffer record_buf(static_cast<std::size_t>(rec_bytes + pad_bytes));
  auto read_full_result = co_await read_full(file_buf_reader, record_buf.asio_mutable_buffer());
  if (!read_full_result) {
    ...
    if (auto validate_crc_result = pb::validate_rec_crc(rec, crc_); !validate_crc_result) {
      ...
      if (is_torn_entry(record_buf)) {
        LOG_ERROR(logger_, "{}: in file '{}' at position: {}", make_error_code(io_error::UNEXPECTED_EOF).message(),
                  file_buf_reader->name(), last_valid_off_);
        co_return tl::unexpected(io_error::UNEXPECTED_EOF);
      }
      LOG_ERROR(logger_, "{}: in file '{}' at position: {}, expected crc: {}, record crc: {}",
                validate_crc_result.error().message(), file_buf_reader->name(), last_valid_off_,
