...
leaf::result<fileutil::file_endpoint> create_new_wal_file(asio::any_io_executor executor, const std::string& filepath,
                                                          bool force_new) {
  asio::error_code ec;
  ...
  // --- 第一步：初始创建 ---
  {
    ...
    BOOST_LEAF_AUTO(first_create, fileutil::create_file_endpoint(executor, filepath, open_flags));
  }

  assert(fileutil::path_exist(filepath));
  // --- 第二步：跨平台设置权限 (0600) ---
  std::error_code perms_ec;
  fs::permissions(filepath, fs::perms::owner_read | fs::perms::owner_write, fs::perm_options::replace, perms_ec);

  if (perms_ec) {
    LOG_ERROR("Failed to set private permissions on file {}: {}", filepath, perms_ec.message());
  ...
  ec = final_stream.open(filepath, asio::file_base::read_write, ec);
  if (ec) {
    LOG_ERROR("Failed to re-open file {} after permission change: {}", filepath, ec.message());
    return new_error(ec, "Failed to re-open file after permission change");
  }

  return fileutil::create_file_endpoint(executor, filepath, asio::file_base::read_write);
}
