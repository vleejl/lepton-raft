...
TEST(asio_coroutine_test_suit, asio_io_context) {
  io_context io;
  lepton::coro::channel<int> proc_chan(io.get_executor());
...
TEST(asio_coroutine_test_suit, asio_io_context2) {
  ...
  lepton::coro::channel<int> chan(excutor, 1);
...
TEST(asio_coroutine_test_suit, async_select_done_with_raft_message_type) {
  ...
  co_spawn(
      io,
      [&]() -> awaitable<void> {
        auto msg = co_await lepton::coro::async_select_done_with_value(
            [&](auto token) {
...
TEST(asio_coroutine_test_suit, async_select_done_with_expected_error_type) {
  // 初始化组件
  asio::io_context io;
  lepton::coro::channel<std::error_code> recvc(io.get_executor());  // 带缓冲的通道
  lepton::coro::signal_channel done_chan(io.get_executor());        // 用于取消的信号通道
...
TEST(asio_coroutine_test_suit, close_channel) {
  // 初始化组件
  asio::io_context io;
  lepton::coro::channel<std::error_code> recvc1(io.get_executor());  // 带缓冲的通道
  lepton::coro::channel<std::error_code> recvc2(io.get_executor());  // 带缓冲的通道
  lepton::coro::signal_channel done_chan(io.get_executor());         // 用于取消的信号通道
...
TEST(asio_coroutine_test_suit, cancellation_signal) {
  ...
  lepton::coro::channel_endpoint<std::string> ch1(io.get_executor());
  lepton::coro::channel_endpoint<std::string> ch2(io.get_executor());
...
TEST(asio_coroutine_test_suit, parallel_send_msg) {
  asio::io_context io;
  lepton::coro::channel<std::string> recvc1(io.get_executor());  // 带缓冲的通道
  lepton::coro::channel<std::string> recvc2(io.get_executor());  // 带缓冲的通道
...
asio::awaitable<void> parallel_process(lepton::coro::channel<std::string>& recvc,
...
TEST(asio_coroutine_test_suit, lepton_parallel_send_msg) {
  asio::io_context io;
  lepton::coro::channel<std::string> recvc1(io.get_executor());
  lepton::coro::channel<std::string> recvc2(io.get_executor());
  lepton::coro::signal_channel done_chan(io.get_executor());
  lepton::coro::signal_channel token_chan(io.get_executor());
...
TEST(asio_coroutine_test_suit, async_send_msg_with_done_signal) {
  asio::io_context io;
  lepton::coro::channel<std::string> ready_chan_(io.get_executor());
  lepton::coro::signal_channel token_chan(io.get_executor());
...
TEST(asio_coroutine_test_suit, async_send_msg_with_done_signal1) {
  ...
  lepton::coro::channel<std::string> ready_chan_(io.get_executor(), /*capacity=*/0);
  lepton::coro::signal_channel cancel_ready_chan(io.get_executor());
  lepton::coro::signal_channel token_chan(io.get_executor());
