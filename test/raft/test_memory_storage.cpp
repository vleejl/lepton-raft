...
auto constexpr MAX_UINT64 = std::numeric_limits<std::uint64_t>::max();
...
TEST_F(memory_storage_test_suit, test_storage_term) {
  ...
  for (const auto &iter_test : tests) {
    ...
    GTEST_ASSERT_EQ(iter_test.wterm, result.value());
  };
}
...
TEST_F(memory_storage_test_suit, test_storage_entries) {
  ...
  std::vector<test_case> tests = {
      {2, 6, MAX_UINT64, make_error_code(storage_error::COMPACTED)},

      {3, 4, MAX_UINT64, make_error_code(storage_error::COMPACTED)},

      {4, 5, MAX_UINT64, EC_SUCCESS, create_entries({{4, 4}})},

      {4, 6, MAX_UINT64, EC_SUCCESS, create_entries({{4, 4}, {5, 5}})},

      {4, 7, MAX_UINT64, EC_SUCCESS, create_entries({{4, 4}, {5, 5}, {6, 6}})},
