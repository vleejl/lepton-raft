...
struct state_machine_builer_pair {
  ...
  state_machine_builer_pair(std::unique_ptr<lepton::core::raft> &&raft_handle_param)
      : raft_handle_ptr(std::move(raft_handle_param)) {
    builder_view = raft_handle_ptr.get();
    raft_handle = raft_handle_ptr.get();
  }

  state_machine_builer_pair(lepton::core::raft &raft_ref) {
  ...
  std::unique_ptr<lepton::core::raft> raft_handle_ptr;
  lepton::core::raft *raft_handle;
};
...
// 内存存储相关
lepton::core::memory_storage new_test_memory_storage(
    std::vector<std::function<void(lepton::core::memory_storage &)>> &&options);
std::function<void(lepton::core::memory_storage &)> with_learners(lepton::core::pb::repeated_uint64 &&learners);

// 状态验证
struct test_expected_raft_status {
  lepton::core::raft *raft_handle;
  lepton::core::state_type expected_state;
...
// 日志操作
lepton::core::pb::repeated_entry next_ents(lepton::core::raft &r, lepton::core::memory_storage &s);
void must_append_entry(lepton::core::raft &r, lepton::core::pb::repeated_entry &&ents);
...
// 特殊状态构造
using config_hook = std::function<void(lepton::core::config &)>;
...
// 配置钩子
void raft_config_quorum_hook(lepton::core::config &cfg);
void raft_config_pre_vote(lepton::core::config &cfg);
void raft_config_read_only_lease_based(lepton::core::config &cfg);

// Raft 钩子
void raft_quorum_hook(lepton::core::raft &sm);
void raft_pre_vote_hook(lepton::core::raft &sm);
void raft_read_only_lease_based_hook(lepton::core::raft &sm);
void raft_become_follower_hook(lepton::core::raft &sm);

// 网络初始化
network init_network(std::vector<std::uint64_t> &&ids, std::vector<config_hook> raft_config_hook = {},
                     std::vector<std::function<void(lepton::core::raft &)>> raft_hook = {});
...
auto ignore_size_hint_mem_storage_entries(lepton::core::memory_storage &, std::uint64_t, std::uint64_t, std::uint64_t);
