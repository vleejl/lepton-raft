...
TEST_F(byte_span_test, from_const_void_ptr) {
  lepton::byte_span span(test_data_.data(), test_data_.size());
...
TEST_F(byte_span_test, from_non_const_void_ptr) {
  ...
  lepton::byte_span span(mutable_data.data(), mutable_data.size());
...
TEST_F(byte_span_test, from_std_string) {
  lepton::byte_span span(test_string_);
...
TEST_F(byte_span_test, from_std_string_view) {
  lepton::byte_span span(test_string_view_);
...
TEST_F(byte_span_test, from_std_vector_byte) {
  lepton::byte_span span(test_data_);
...
TEST_F(byte_span_test, from_std_vector_uint8) {
  lepton::byte_span span(uint8_data_);
...
TEST_F(byte_span_test, empty_containers) {
  ...
  lepton::byte_span span1(empty_vec);
  EXPECT_EQ(span1.size(), 0);

  // 空 string
  std::string empty_str;
  lepton::byte_span span2(empty_str);
  ...
  lepton::byte_span span3(const_nullptr, 0);  // 明确调用 const void* 版本
  EXPECT_EQ(span3.size(), 0);

  lepton::byte_span span4(mutable_nullptr, 0);  // 明确调用 void* 版本
